...
Use `INVEST` or `PASS` and an integer score from 0 to 100. Omit `proposedTerms` for a pass; for an investment, propose only simple portfolio-demo terms such as check size, ownership target, and one diligence condition.
